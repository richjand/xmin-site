...
We'll set our number of teams to be 30 and the number of games to be 16. $\alpha$ is set to three and can be thought of as the built-in advantage for team 1 which in a real application would be something like a home field advantage, assuming team 1 is the home team. $\sigma$ is the amount of variance in the score difference given the ability of the two teams, $\sigma_{start}$ is the amount of variance in team ability in week 1, and $\sigma_{state}$ is the week to week variance in team ability.
...
print(gen_mu_ts(n_weeks, mu_start[1]))
```
...
fit <- sampling(stan_mod, data = standat, chains = 4, iter = 1000, cores = 2)
